...
NLE Edit operations (Master Spec Section 27):
  ...
  - Ripple Delete: Trim/delete clip and shift all downstream clips on the track.
...
class EditorialCommandEngine:
    ...
    def slip_edit(
        ...
        """
        Slip Edit: Shift internal media content within fixed timeline position and duration.
        Returns updated clip with adjusted label / media offset.
        """
        logger.debug("slip_edit: clip %s offset=%dms", clip.clip_id, offset_ms)
        return clip.model_copy()

    @staticmethod
    def slide_edit(
        left_clip: TimelineClip,
        mid_clip: TimelineClip,
        right_clip: TimelineClip,
        shift_ms: int,
    ) -> tuple[TimelineClip, TimelineClip, TimelineClip]:
        """
        Slide Edit: Move mid_clip along timeline while automatically trimming/extending
        preceding left_clip and following right_clip.
        """
        new_start = mid_clip.start_ms + shift_ms
        if new_start <= left_clip.start_ms or new_start + mid_clip.duration_ms >= right_clip.end_ms:
            raise ValueError("slide edit exceeds bounds of adjacent clips")

        new_left = left_clip.model_copy(update={"duration_ms": new_start - left_clip.start_ms})
        new_mid = mid_clip.model_copy(update={"start_ms": new_start})
        new_right = right_clip.model_copy(
            update={
                "start_ms": new_start + mid_clip.duration_ms,
                "duration_ms": right_clip.end_ms - (new_start + mid_clip.duration_ms),
            }
        )
        return new_left, new_mid, new_right
